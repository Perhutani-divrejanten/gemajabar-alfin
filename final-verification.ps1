# Final Verification Script - Memastikan rebrand Gema Jabar selesai dengan baik

$WorkspaceRoot = "c:\KULIAH\MAGANG\Magang di Perhutani\Gema Jabar"
...
$oldBrandingPatterns = @("Gema Jabar", "gemajabar", "Gema Jabar")
...
# 2. Check for text logo Gema Jabar references
Write-Host "2. Checking for text logo Gema Jabar image references..." -ForegroundColor Yellow
...
if ($logoFound) {
    ...
    Write-Host "   ⚠️  Found $($logoFound.Count) references to text logo Gema Jabar image" -ForegroundColor Yellow
    foreach ($ref in $logoFound | Select-Object -First 5) {
        Write-Host "      - $($ref.Path | Split-Path -Leaf)" -ForegroundColor Yellow
    }
} else {
    Write-Host "   ✅ No text logo Gema Jabar image references found!" -ForegroundColor Green
}
...
$newColors = @("#9A3412", "#9A3412", "#7F2B2B")
$colorsFound = 0
...
Write-Host "4. Checking for new branding 'Gema Jabar'..." -ForegroundColor Yellow
$newBrandingFound = Get-ChildItem -Path $WorkspaceRoot -Recurse -Include "*.html" -File |
    Select-String -Pattern "Gema Jabar|Gema Jabar|gemajabar" -ErrorAction SilentlyContinue |
    Where-Object { $_.Path -notlike "*\.bak.*" } | 
    Measure-Object

if ($newBrandingFound.Count -gt 0) {
    Write-Host "   ✅ Found 'Gema Jabar' branding in $($newBrandingFound.Count) places" -ForegroundColor Green
} else {
    Write-Host "   ⚠️  No 'Gema Jabar' branding found!" -ForegroundColor Yellow
    $issues += @{ Type = "Missing"; File = "All"; Reason = "No Gema Jabar branding found" }
}
...
foreach ($pkg in $pkgFiles) {
    ...
    if ($content -match '"name"\s*:\s*"gemajabar') {
        $pkgOK++
...
Write-Host ""
Write-Host "Rebrand Gema Jabar SELESAI" -ForegroundColor Green
